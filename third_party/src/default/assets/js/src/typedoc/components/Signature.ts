module typedoc {
  ...
  /**
   * Controls the tab like behaviour of methods and functions with multiple
   * signatures.
   */
  ...
  class Signature extends Backbone.View<any> {
    ...
    private createGroups() {
      const $signatures = this.$el.find('> .tsd-signature')
      if ($signatures.length < 2) return

      this.$container   = this.$el.siblings('.tsd-descriptions')
      const $descriptions = this.$container.find('> .tsd-description')
